 ...
 * PEM-only ({@code certificate.file} / {@code certificate.key.file}). PF-68 dropped
 * the JKS keystore branch since every TLS configuration JKS expressed had a PEM
 * equivalent and the local-dev mkcert workflow is PEM-native. Uses
 * {@link Http3#supportedApplicationProtocols()} so the ALPN advertisement always
 * tracks whatever h3 draft Netty currently negotiates.
 *
