...
Enable the **Secure** module in the application's @build.gradle.kts@ and restart the application.
...
That’s fun but when we go to the CRUD part of the administration area, we lose the administration layout. That’s because the CRUD module provides its own layout. But we can of course override it by copying the module's layout into our app:

bc. cp $PLAY_HOME/modules/crud/app/views/CRUD/layout.html app/views/CRUD/layout.html

The template loader prefers app-level files, so this new @/yabe/app/views/CRUD/layout.html@ takes effect immediately. Replace its content to integrate it nicely with our @admin.html@ layout:
...
The administration area is almost graphically coherent now. The last thing to style is the log-in page. As always it is easily customizable by copying the default CSS from the secure module into the application:

bc. cp $PLAY_HOME/modules/secure/public/stylesheets/secure.css public/stylesheets/secure.css
