...
In the application's @build.gradle.kts@, enable the Secure module:
...
h2. <a name="commands">Customizing the log-in pages</a>

The Secure module ships default templates for the log-in page, its layout, and
the accompanying CSS. To customize any of them, copy the file from the module
into your application — the template loader prefers app-level files over
module-level ones.
...
bc. $ cp $PLAY_HOME/modules/secure/app/views/Secure/login.html app/views/Secure/login.html

h3. <a name="layout">Main layout override</a>

bc. $ cp $PLAY_HOME/modules/secure/app/views/Secure/layout.html app/views/Secure/layout.html
