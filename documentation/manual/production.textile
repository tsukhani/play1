...
h2. <a name="jdk">JDK requirement</a>

Play 1.12 requires **JDK 25 or later** at runtime. Build your application targeting Java 25 (e.g. @--release 25@ if you compile with @javac@ directly). Older JDKs are not supported — the framework relies on Java 25 features including JEP 491 (virtual threads no longer pin the carrier thread under @synchronized@).

h2. <a name="server-narrative">Deployment model</a>

Play 1.12 deploys as a stand-alone Netty server — there is no servlet/WAR path. Production is a @play start@ process (typically running behind a reverse proxy that terminates TLS / HTTP/2 / HTTP/3 and load-balances). For the underlying HTTP server's capabilities (HTTP/2, HTTP/3, ALPN, virtual-thread dispatch), see "the HTTP server":httpserver. For deployment topology and process supervision, see "deployment":deployment.
...
PF-68 made HTTPS PEM-only. Configure the cert+key paths in your @application.conf@ file:

bc. # X509 certificates (PEM)
certificate.file=certs/host.cert
certificate.key.file=certs/host.key

The values above are the framework defaults. Keys must be unencrypted PEM — Netty's @SslContextBuilder@ does not accept encrypted keys.

For local development, generate a cert+key with @mkcert@ (preferred — produces a system-trusted cert that works with Chrome's HTTP/3 stack) or @openssl@ as a fallback (self-signed; browsers warn and HTTP/3 won't upgrade). See "TLS configuration":httpserver#local-dev for the exact commands.

For production, replace @certs/host.cert@ and @certs/host.key@ with a real cert from Let's Encrypt, an internal CA, or your usual certificate provider. Generate self-signed certs manually with:

bc. openssl req -x509 -newkey rsa:2048 -nodes \
    -keyout host.key -out host.cert \
    -days 3650 -subj '/CN=your-host'
