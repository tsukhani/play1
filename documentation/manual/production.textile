...
For local development, the easiest path is the "play enable-https":commands#enable-https command, which generates a cert+key pair via @mkcert@ (preferred — produces a system-trusted cert that works with Chrome's HTTP/3 stack) or @openssl@ (fallback — self-signed).
