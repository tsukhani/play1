...
To enable the CRUD module, declare it in the application's @build.gradle.kts@:

bc. play1 {
    modules("crud")
}

The framework's bundled modules (@crud@, @secure@, @testrunner@, @docviewer@) are
resolved from the framework distribution. Re-import the Gradle project in
IntelliJ IDEA to refresh source roots.
...
To override a CRUD template, copy the corresponding file from the CRUD module
into your application's @app/views/@. The template loader prefers app-level
files over module-level ones, so this is enough — no rewiring required.

bc. $ cp $PLAY_HOME/modules/crud/app/views/CRUD/list.html app/views/Comments/list.html
...
bc. $ cp $PLAY_HOME/modules/crud/app/views/CRUD/show.html app/views/Posts/show.html
