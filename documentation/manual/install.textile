...
To run the Play framework, you need "Java 25 or later":https://adoptium.net/temurin/releases. If you wish to build Play from source, you will need the "Git source control client":http://git-scm.com/ to fetch the source code and "Ant":http://ant.apache.org/ to build it.
...
The *play* command line utility is a thin shell wrapper around Gradle. It works on macOS and Linux out of the box; the wrapper locates the project's @gradlew@ (or a system @gradle@ on PATH) and translates classic Play 1 CLI flags to Gradle properties.
...
# Edit @/etc/paths@ and add the line @/Applications/play-1.13.0@ (for example).
...
To install Java, just download and install the latest JDK package.
...
bc. # git clone https://github.com/tsukhani/play1.git
# cd play1/framework
# ant jar
