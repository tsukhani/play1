...
h2. <a name="openapi">Generating OpenAPI from your routes</a>

The framework auto-generates an "OpenAPI 3 spec":openapi from your @conf/routes@ file plus reflection on your controller method signatures. The spec is exposed at @/@api/openapi.json@ (and @/@api/openapi.yaml@), with an interactive Swagger UI at @/@api/docs@ in development mode. Most apps need no annotations to get a useful spec — the routes file alone is enough — but you can decorate controllers and actions with @io.swagger.v3.oas.annotations.*@ to enrich the output. See "OpenAPI 3 spec generation":openapi for the full reference.
