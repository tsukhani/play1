...
The route serves every HTTP version the server speaks. Over HTTP/1.1 the socket is established with the classic @Upgrade: websocket@ handshake; over HTTP/2 and HTTP/3 clients may instead bootstrap it with Extended CONNECT, so the WebSocket multiplexes onto the connection they already have. Your controller is identical either way — see "WebSockets over HTTP/2 and HTTP/3":httpserver#websockets-over-h2-h3 for the transport detail.
...
Of course usually you don’t want to close the socket immediately. To keep it open, iterate the @inbound@ channel: each step blocks the current (virtual) thread until the next event arrives, and the loop ends once the socket closes.

@inbound@ delivers a sealed @Http.WebSocketEvent@ — one of @TextFrame@, @BinaryFrame@ or @WebSocketClose@ — so a @switch@ over it is exhaustive: the compiler checks that you have handled every case.
    ...
    public static void echo() {
        for (WebSocketEvent event : inbound) {
            switch (event) {
                ...
                case TextFrame(var msg)    -> outbound.send("Echo: %s", msg);
                case BinaryFrame(var data) -> outbound.sendBinary(data);
                case WebSocketClose closed -> Logger.info("Socket closed!");
            }
        }
    }
...
Record patterns deconstruct the frame inline (@TextFrame(var msg)@), and a guarded @case@ (@when msg.equals("quit")@) handles the special message. Because @WebSocketEvent@ is sealed, adding a new event type later turns every @switch@ that forgot it into a compile error rather than a silent runtime drop. The event types live in @play.mvc.Http@.
