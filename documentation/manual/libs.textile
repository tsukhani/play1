...
* "Codec":/@docs/api/play/libs/Codec.html: Utilities to encode and decode data.
* "Crypto":/@docs/api/play/libs/Crypto.html: Cryptographics utilities.
* "Expression":/@docs/api/play/libs/Expression.html: Evaluate dynamic expressions.
* "F":/@docs/api/play/libs/F.html: Functional programming with Java.
* "Files":/@docs/api/play/libs/Files.html: File system manipulation helpers.
* "I18N":/@docs/api/play/libs/I18N.html: Internationalization helpers.
* "IO":/@docs/api/play/libs/IO.html: Stream manipulation helpers.
* "Images":/@docs/api/play/libs/Images.html: Image manipulation utilities.
* "Mail":/@docs/api/play/libs/Mail.html: E-mail functions.
* "MimeTypes":/@docs/api/play/libs/MimeTypes.html: Dealing with MIME types.
* "OAuth2":/@docs/api/play/libs/OAuth2.html: OAuth 2.0 client protocol.
* "OpenID":/@docs/api/play/libs/OpenID.html: OpenID client protocol.
* "Time":/@docs/api/play/libs/Time.html: Time and duration utilities.
* "WS":/@docs/api/play/libs/WS.html: Powerful Web Service client.
* "XML":/@docs/api/play/libs/XML.html: Loading XML structures.
* "XPath":/@docs/api/play/libs/XPath.html: Parsing XML using XPath.
...
The @play.libs.WS@ provides a powerful HTTP client. Under the hood it uses "OkHttp 5":https://square.github.io/okhttp/, with the dispatcher pointed at a virtual-thread executor so blocking calls scale with carrier threads.
...
"OAuth 2.0":https://oauth.net/2/ is the standard authorization protocol used by every modern provider. It is used by services such as "Facebook":http://developers.facebook.com/docs/authentication/, Google, GitHub, and most SaaS APIs.

The general process is the following:

* Redirect the user to the provider's authorization page
* After the user grants authorization, the provider redirects back to your server with a code
* Your server exchanges the code for an access token specific to the current user, which needs to be saved in order to perform requests to the service. This step is server-to-server.

Play takes care of most of the process.
...
For a working OAuth2 flow, wire @WS@ calls into a controller action: @retrieveVerificationCode()@ in the auth-start action, @retrieveAccessToken()@ in the redirect-back action, then call the resource API with the token.
