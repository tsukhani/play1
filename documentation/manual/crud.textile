...
In the application's @build.gradle.kts@, enable the CRUD module:
...
Open a shell, go the application directory and copy the default CRUD
@list.html@ template into your application:

bc. cp $PLAY_HOME/modules/crud/app/views/CRUD/list.html app/views/Users/list.html

The template loader prefers app-level files over module-level ones, so the
copied template is the one Play will render.
...
h2. <a name="commands">Customizing CRUD templates</a>

The CRUD module loads templates from your application if they are present,
falling back to its own otherwise. To customize a template, copy the
corresponding file from the module into your application's @app/views/@.

h3. Override a per-type template

bc. cp $PLAY_HOME/modules/crud/app/views/CRUD/list.html app/views/Users/list.html

Replace @list.html@ / @Users@ with the template and target type you want.

h3. Override the main layout
