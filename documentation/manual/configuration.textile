...
h2. <a name="envvar">Environment variable interpolation</a>

Any value in @application.conf@ may reference an environment variable using @${ENV_VAR}@ syntax. This is the recommended pattern for secrets, deployment-specific URLs, and anything else that should not be checked into the config file (PF-10).

bc.. application.secret = ${PLAY_SECRET}
db.url = ${DATABASE_URL}
mail.smtp.pass = ${SMTP_PASSWORD}

p. Resolution happens at config load time. The @${...}@ placeholder is replaced with the environment variable's value before any other code reads the configuration, so callers see the resolved string transparently.

To supply a fallback when the environment variable is unset, use @${ENV_VAR:-default}@:

bc.. # Falls back to localhost when DB_HOST isn't exported
db.url = jdbc:postgresql://${DB_HOST:-localhost}:5432/myapp

# Empty default — yields an empty string when LOG_LEVEL is unset
application.log = ${LOG_LEVEL:-INFO}

p. The default may itself be a literal string (it is not recursively interpolated). Whitespace inside @${...}@ is significant — write @${DB_HOST:-localhost}@, not @${ DB_HOST :- localhost }@.

Unresolved placeholders with no default log a @WARN@ at startup and resolve to the literal placeholder string. This is intentional — silent failure on a missing secret is worse than a warn-and-misbehave that surfaces during smoke tests.

Configurations without any @${...}@ syntax behave identically to before — there is no migration needed when upgrading.
...
p(note). Play does not bundle a MySQL JDBC driver. Add your own to the application's dependencies — @com.mysql:mysql-connector-j@ or @org.mariadb.jdbc:mariadb-java-client@ — when using a @mysql:…@ configuration.
...
The default and only built-in factory is @play.db.hikaricp.HikariDataSourceFactory@. Custom factories may be supplied by implementing @play.db.DataSourceFactory@. The class is resolved with the application classloader, so it may live in a jar on the classpath or directly in your application's @app/@ tree.
...
Milliseconds. Maps to HikariCP's leak-detection threshold. When a connection is checked out for longer than this many milliseconds, HikariCP logs a stack trace pointing to the leak. Zero disables leak detection. HikariCP rejects positive values below 2000.

Default: @0@

h3(#db.pool.maxConnectionAge). db.pool.maxConnectionAge

Milliseconds. Maps to HikariCP's @maxLifetime@: a connection older than this is removed from the pool when it next returns. Zero means connections never expire.

Default: @1800000@ (30 minutes)

h3(#db.pool.maxIdleTime). db.pool.maxIdleTime

Milliseconds a connection can remain pooled but unused before being discarded. Maps to HikariCP's @idleTimeout@. Zero means idle connections never expire.

Default: @600000@ (10 minutes)

h3(#db.pool.maxSize). db.pool.maxSize

Connection pool maximum size. Maps to HikariCP's @maximumPoolSize@. For example:
...
Minimum number of idle connections HikariCP keeps in the pool. Maps to @minimumIdle@. For example:
...
Connection acquisition timeout in milliseconds. Maps to HikariCP's @connectionTimeout@. For example:
...
Sets the SQL string that will be executed on all new connections when they are created, before they are added to the pool.
...
Specifies the web browser compatibility mode for the HtmlUnit headless web browser used when running @play autotest@.
...
This value can be set directly in the command @play autotest@ by adding @--timeout=60@
...
Define allowed methods that will be handled when the request asks for a method override, either through the @X-HTTP-Method-Override@ header or through an @x-http-method-override@ query-string parameter. A method not listed here is ignored on both.

bc. http.allowed.method.override=PUT,DELETE,PATCH

Default: none — method overrides are ignored entirely.

A GET is never rewritten by the query-string form, whatever this setting contains. A cross-site top-level GET navigation is the one cross-site shape that @SameSite=Lax@ still attaches session cookies to, so honouring @GET /order/1?x-http-method-override=DELETE@ would make every state-changing route reachable by CSRF. The header form has no such restriction, because a cross-site navigation cannot set a header.

Note that the @#{form}@ and @#{a}@ tags rely on the query-string form: an HTML form can only emit GET or POST, so for a PUT, DELETE or PATCH action the tags append @x-http-method-override@ to the URL and submit a POST. Applications that route such actions through those tags must list the method here, or the form will POST and the route will not match.
...
The URL path where the application runs on the server: use this if you do not host your Play application at the root of the domain you’re serving it from. For example:
...
h2(#certificate). X509 certificates (PEM)

Play 1.12 ships PEM-only HTTPS support (PF-68 dropped JKS). Generate the cert+key with @mkcert@ (preferred — produces a system-trusted cert that works with Chrome's HTTP/3 stack) or @openssl@ as a fallback (self-signed; browsers warn and HTTP/3 won't upgrade). See "TLS configuration":httpserver#local-dev for the exact commands.


h3(#certificate.file). certificate.file

Path to the PEM-encoded X.509 certificate (or chain).

bc. certificate.file=certs/host.cert
...
Path to the unencrypted PEM-encoded private key. The framework's Netty @SslContextBuilder@ does not accept encrypted keys; @mkcert@ and the @openssl req -nodes@ fallback both produce unencrypted keys.

bc. certificate.key.file=certs/host.key

Default: @certs/host.key@


h2(#cacheconfig). Cache (PF-88)


h3(#cacheprovider). cache.provider

Selects the @play.cache.CacheProvider@ implementation when more than one is registered via @META-INF/services/play.cache.CacheProvider@. The framework ships @caffeine@ as the default and only-bundled provider; this property is only consulted when an app brings additional providers (EhCache, Infinispan, a no-op for tests). Value must match a provider's @name()@.

bc. cache.provider=caffeine

Default: unset (single-provider classpath uses the only registered provider; multi-provider classpath without this key fails fast at boot)

The legacy @memcached=enabled@ / @memcached.host=...@ keys are gone — memcached support was removed in 1.12.39. See "using a cache":cache for the full SPI story.
...
h3(#mail.smtp.ssl.cavalidation). mail.smtp.ssl.cavalidation

Whether to verify the SMTP server's certificate. Applies to both encrypted values of "mail.smtp.channel":#mail.smtp.channel. When enabled, the certificate chain must be trusted **and** must match the host being dialled.

Set this to @false@ only for a server whose certificate you cannot fix. Play logs a warning naming the host at startup whenever it is disabled. What the opt-out gives up differs by channel:

* @ssl@ - *any* certificate is accepted, including self-signed. Anyone able to intercept the connection can read the message and the SMTP AUTH credentials sent after the handshake.
* @starttls@ - the chain is still verified, but the certificate is no longer checked against the hostname, so a trusted certificate issued for any other domain is accepted.

bc. mail.smtp.ssl.cavalidation=false

Default: @true@

p(note). Play 1.13.48 and earlier never verified the certificate on the @ssl@ channel, and never checked the hostname on @starttls@. If you upgrade and mail stops sending, the server's certificate is either untrusted by the JVM or does not name the host you connect to — fix the certificate or install it in the truststore rather than reaching for this property.
...
Open file from error pages. If your text editor supports opening files by URL, Play will dynamically link error pages to files. For IntelliJ IDEA, for example:

bc. play.editor=idea://open?file=%s&line=%s
...
Maximum HTTP request body size, in bytes. Requests above this cap are rejected with @413 Request Entity Too Large@ and the connection is closed.

bc. play.netty.maxContentLength=10485760

Default: @-1@ (unlimited).

p(note). *Semantic change in 1.12.0:* this property was previously a per-chunk size cap inside @HttpChunkAggregator@ (Netty 3). Under Netty 4 it is now a hard request-body cap enforced by @StreamChunkAggregator@. If you previously set this to e.g. @1048576@ thinking of it as "8 KB chunks aggregated up to 1 MB", review whether that limit is still appropriate as an absolute body cap.


h3(#play.netty.spoolThresholdBytes). play.netty.spoolThresholdBytes

Threshold above which an incoming request body is spooled to a temp file under @play.tmp@ instead of accumulating in memory. Bodies smaller than this stay in memory.

bc. play.netty.spoolThresholdBytes=1048576

Default: @1048576@ (1 MB).

Tuning:

* Upload-heavy workloads (concurrent multi-MB uploads): drop to @65536@ to cap heap pressure
* Low-concurrency JSON APIs: raise to @4194304@ if you want to keep more in memory
...
The standard name of a Java Secure Socket Extension (JSSE) key management algorithm. Read by the framework when constructing the inbound HTTPS @SslContext@ (PF-68: PEM-only).
