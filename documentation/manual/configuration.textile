...
h3(#play.threads.virtual). play.threads.virtual

Run framework subsystems on virtual threads (JEP 444). Off by default; the fork ships the machinery but does not enable it for you.

bc. play.threads.virtual=true

Default: @false@.

When enabled, request invocation, background jobs, and mail dispatch each run on a virtual-thread executor. Per-subsystem overrides (any of which inherits from @play.threads.virtual@ when unset):

* @play.threads.virtual.invoker@ — request invocation only
* @play.threads.virtual.jobs@ — background jobs (@@Every@, @@On@) only
* @play.threads.virtual.mail@ — mail delivery only
