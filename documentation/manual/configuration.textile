...
Play 1.12 ships PEM-only HTTPS support (PF-68 dropped JKS). The "play enable-https":commands#enable-https command generates these files automatically via @mkcert@ (preferred — produces a system-trusted cert that works with Chrome's HTTP/3 stack) or @openssl@ (fallback — self-signed).
