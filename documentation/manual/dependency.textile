...
h3. <a name="core-chain">Play core resolver chain</a>

For the framework's own core dependencies, Play uses a chain resolver named @playCoreDependencies@: it tries @playCoreDependenciesLocal@ first (offline jars in @framework/lib/@) and falls back to @playCoreDependenciesCentral@ (Maven Central) when a coordinate is missing locally. This means new dependencies you add to @framework/dependencies.yml@ (or transitive dependencies of existing ones) are auto-fetched from Maven Central on the first @ant resolve@ — there is no need to manually pre-seed @framework/lib/@ before resolving. Application-level @conf/dependencies.yml@ resolution still uses the standard Maven Central + Play modules repositories described above.
