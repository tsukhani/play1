...
もちろん、通常は直ちにソケットを閉じたいと思わないでしょう。ソケットを開いたままにするには、@inbound@ チャネルを反復処理します。各ステップは次のイベントが到着するまで現在の (仮想) スレッドをブロックし、ソケットが閉じるとループが終了します。

@inbound@ は封印された (sealed) @Http.WebSocketEvent@ — @TextFrame@、@BinaryFrame@、@WebSocketClose@ のいずれか — を返すため、これに対する @switch@ は網羅的になります。すべてのケースを処理しているかどうかをコンパイラが検査します。
...
レコードパターンはフレームをインラインで分解し (@TextFrame(var msg)@)、ガード付きの @case@ (@when msg.equals("quit")@) が特別なメッセージを処理します。@WebSocketEvent@ は封印されているため、後で新しいイベントタイプを追加すると、それを処理し忘れた @switch@ は実行時に黙って破棄されるのではなくコンパイルエラーになります。イベントタイプは @play.mvc.Http@ にあります。
