...
CRUD モジュールを有効にするには、アプリケーションの @build.gradle.kts@ で宣言します:

bc. play1 {
    modules("crud")
}

フレームワークにバンドルされたモジュール (@crud@、@secure@、@testrunner@、@docviewer@) はフレームワーク配布物から解決されます。IntelliJ IDEA を使っている場合は、ソースルートを更新するために Gradle プロジェクトを再インポートしてください。
...
CRUD のテンプレートを上書きするには、対応するファイルを CRUD モジュールからアプリケーションの @app/views/@ にコピーします。テンプレートローダーはモジュールレベルのものよりもアプリケーションレベルのファイルを優先するため、これだけで十分です。再配線は必要ありません。
