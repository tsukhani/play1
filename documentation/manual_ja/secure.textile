...
アプリケーションの @build.gradle.kts@ で Secure モジュールを有効にします:
...
h2. <a name="commands">ログインページのカスタマイズ</a>

Secure モジュールには、ログインページ、そのレイアウト、付随する CSS のデフォルトテンプレートが同梱されています。いずれかをカスタマイズするには、モジュールから対応するファイルをアプリケーションにコピーしてください。テンプレートローダーはモジュールレベルのものよりもアプリケーションレベルのファイルを優先します。
